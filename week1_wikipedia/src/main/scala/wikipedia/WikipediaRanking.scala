...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    def langOccr(article: WikipediaArticle, lang: String): Int = if (article.mentionsLanguage(lang)) 1 else  0
    rdd.aggregate(0)((sum, article) => sum + langOccr(article, lang), _+_)
  }
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    val ranks: RDD[(String, Int)] = for (pair <- index) yield (pair._1, pair._2.size)
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    val list: RDD[(String, Int)] = rdd.flatMap(article => for (lang <- langs if article.mentionsLanguage(lang)) yield (lang, 1))
