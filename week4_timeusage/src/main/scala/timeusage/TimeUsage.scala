...
object TimeUsage extends TimeUsageInterface {
  ...
  def row(line: List[String]): Row = {
    val res = line.head :: line.tail.map(_.toDouble)
    Row.fromSeq(res)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primary_prefix = List("t01", "t03", "t11", "t1801", "t1803")
    val primary = List[Column]()

    val working_prefix = List("t05", "t1805")
    val working = List[Column]()

    val other_prefix = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")
    val other = List[Column]()

    def acc(column: List[String], p: List[Column], w: List[Column], o: List[Column]) : (List[Column], List[Column], List[Column]) =
      column match {
        case Nil => (p, w, o)
        case x::xs => {
          if (primary_prefix.exists(prefix => x.startsWith(prefix))) acc(xs, col(x)::p, w, o)
          else if (working_prefix.exists(prefix => x.startsWith(prefix))) acc(xs, p, col(x)::w, o)
          else if (other_prefix.exists(prefix => x.startsWith(prefix))) acc(xs, p, w, col(x)::o)
          else acc(column, p, w, o)
        }
      }

    acc(columnNames, primary, working, other)
  }
  ...
  def timeUsageSummary(
    ...
    val workProjection: Column = (sum_udf(workColumns, lit(0.0d)) / 60.0).as("work")
    val otherProjection: Column = (sum_udf(otherColumns, lit(0.0d)) / 60.0).as("other")
    df
