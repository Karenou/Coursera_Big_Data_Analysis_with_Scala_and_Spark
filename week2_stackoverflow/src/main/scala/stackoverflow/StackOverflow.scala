...
class StackOverflow extends StackOverflowInterface with Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    ...
    val question = postings.filter(post => post.postingType == 1).map(post => (post.id, post))
    val answer = postings.filter(post => post.postingType == 2)
...
//    grouped.flatMap(x => x._2).groupByKey().mapValues(x => answerHighScore(x.toArray))
  ...
  def computeMedian(a: Iterable[(Int, Int)]) = {
    ...
    val length = s.length
    val (lower, upper) = s.sorted.splitAt(length / 2)
